...
strLeft = ""
def Callback_split_tcp(packet):
    #print(1)
    global strLeft
    try:
        if not packet[IP].proto==6:
            return
        bytePayload =packet[TCP].payload.original
        length = len(bytePayload)
        if length==0:
            return
        #print(len(bytePayload))
        strData = bytePayload.decode('utf8')
        #print(strData)
        subStrs = (strLeft + strData).split()
        if len(subStrs)>0:
            strLeft = subStrs[-1] if strData[-1]!=" " else ""
            print('seq',subStrs[0],'length',length,'time',Utils.getStrTime())
    except:
        return 
...
if __name__=="__main__":
    ...
    if packet_limit>0:
        ...
        if args.t==2:
            while True:
                sniff(count=max_cnt,filter='dst host 10.0.1.1',iface=iface, prn=Callback_split_tcp) #tcp packet from client to server
                time.sleep(2)
        elif args.t==1:
            while True:
